...
4.  **Predict** customer segment membership interactively using decision tree and random forest models, with adjustable parameters exposed through the Shiny interface.
...
Two R packages were evaluated for the classification module: `rpart` (pruned classification tree) and `ranger` (random forest). `rpart` was selected as the primary model for its interpretability — the tree structure allows users to visually trace how splitting rules on variables such as `avg_tx_value` and `tx_count` lead to segment predictions. `ranger` was included as a comparison model as it achieved marginally higher accuracy (99.28% vs 99.22%) through ensemble learning, though at the cost of interpretability. Both models are exposed in the Shiny app with interactive parameter controls — complexity parameter, maximum depth, and number of trees — so users can tune and compare them directly.
